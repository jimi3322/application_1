...
public class NotificationUtils {
    ...
    /**
     * 通知渠道是Google在Android O中新增的功能或是增强，把震动、音效和灯效等相关效果放在了
     * 通知渠道中控制，这样用户就可以有选择的控制应用某一类通知的通知效果。
     * */
    @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
...
//        NotificationManager.IMPORTANCE_HIGH 有声音震动 状态栏显示应用的通知图标(smallIcon) 并且会有heads-up的浮动通知效果
        ...
        private String channelGroupId;                  //通知渠道组ID
        private String channelGroupName;                //通知渠道组名
        private String channelId;                       //通知渠道ID
        private String channelDesc;                     //通知渠道具体描述
        private String channelName;                     //通知渠道名
        private boolean byPassDnd = false;              //是否绕过免打扰
        private int visibility = Notification.VISIBILITY_PUBLIC;        //是否在锁屏界面显示
        private boolean showBadge = false;              //是否在桌面图标显示未读消息数量
        private int importance = NotificationManager.IMPORTANCE_DEFAULT;        //类型 具体看上面

        private boolean enableSound = true;             //声音
        private boolean enableVibrate = true;           //震动
        private boolean enableLight = true;             //灯效
        private Uri soundUri;                           //提示音
        private long[] vibrateLong;                     //震动时长
        private int lightColor = -1;                    //灯效颜色
