...
const val TAG = "HistoricalDataActivity"
const val PAGE_COUNT = 10

@Suppress("NAME_SHADOWING")
class HistoricalDataActivity : BaseActivity() {
    ...
    private fun initItemView(){
        ...
        start_date.setExpandListener(object : ChooseView.OnChooseClickListener{
            override fun onExpand() {
                TimePickerBuilder(this@HistoricalDataActivity,OnTimeSelectListener {
                    date, _ ->
...
//                stationDialog?.dismiss()
        ...
        historyPoint.setExpandListener(object : ChooseView.OnChooseClickListener{
            override fun onExpand() {
                if (mProvinces.size == 0){
                    Toast.makeText(this@HistoricalDataActivity, "站点信息还没有获取到 稍后再试", Toast.LENGTH_SHORT).show()
                    historyPoint.setExpand(false)
                }else{
                    stationDialog = ChoosePointDialog(this@HistoricalDataActivity
        ...
        monitorItems.setExpandListener(object : ChooseView.OnChooseClickListener{
            override fun onExpand() {
                if (mMonitorItems.size == 0){
                    Toast.makeText(this@HistoricalDataActivity, "监测信息还没有获取到 稍后再试", Toast.LENGTH_SHORT).show()
                    historyPoint.setExpand(false)
                }else{
                    monitorDialog = MulipleDialog(this@HistoricalDataActivity
                        ...
                        override fun onResult(result: ArrayList<BaseChooseItemType>, mCounts:ArrayList<Int>) {
                            this@HistoricalDataActivity.mCounts = mCounts
                            ...
                            //监测表头数据填充
                            mselectCounts = mselectMonitorHeader.size
                            val headerManager = GridLayoutManager(this@HistoricalDataActivity,mselectCounts)
                            recyclerHeader.layoutManager = headerManager
                            mHeaderAdapter = HeaderAdapter(this@HistoricalDataActivity)
        ...
        getDatas.setOnClickListener {
            if(mselectMonitorItems.isEmpty()){
                Toast.makeText(this@HistoricalDataActivity
                        , this@HistoricalDataActivity.resources.getString(R.string.please_select_monitorItems)
    ...
    private fun initRecyclerView() {
        ...
        recyclerhisview.addItemDecoration(SpaceItemDecoration(bottom = 5))
        ...
        OkHttpUtils.getInstance()
                .post(this, HttpUtil.getAirServiceAddress(AirConstants.GET_AIR_STATION_TREE)
                    ...
                    override fun onSuccess(statusCode: Int, d: WanResponse<ArrayList<TestPoint>>?) {
                        Toast.makeText(this@HistoricalDataActivity
                                , this@HistoricalDataActivity.resources.getString(R.string.get_stations_success)
                                , Toast.LENGTH_SHORT).show()
                        handlePoint(d)
                        initTableHeaderInfo()
                    }
                    ...
                    override fun onFailure(statusCode: Int, error_msg: String?) {
                        ...
                        Toast.makeText(this@HistoricalDataActivity
                                , this@HistoricalDataActivity.resources.getString(R.string.get_stations_fail)
                                , Toast.LENGTH_SHORT).show()
                    }
                })
    }
        ...
        OkHttpUtils.getInstance()
                .post(this, HttpUtil.getAirServiceAddress(AirConstants.GET_TABLE_HEADER_DATA)
                    ...
                    override fun onSuccess(statusCode: Int, d: WanResponse<ArrayList<TabelHeaderData>>?) {
                        Toast.makeText(this@HistoricalDataActivity
                                , this@HistoricalDataActivity.resources.getString(R.string.get_monitorItems_success)
                                , Toast.LENGTH_SHORT).show()
                        handleTabelHeader(d)
                    }
                    override fun onFailure(statusCode: Int, error_msg: String?) {
                        Log.i(TAG, "onFailure: 站点信息获取失败")
                        Toast.makeText(this@HistoricalDataActivity
                                , this@HistoricalDataActivity.resources.getString(R.string.get_monitorItems_fail)
                                , Toast.LENGTH_SHORT).show()
                    }
                })
    }
    ...
    override fun initToolbar() {
        toolbar.title.text = resources.getString(R.string.history_data)
